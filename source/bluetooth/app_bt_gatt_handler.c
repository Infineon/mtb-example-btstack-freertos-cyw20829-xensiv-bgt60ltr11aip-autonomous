...
app_gatt_connect_handler(wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    else
    {
         ...
         * will be off
         */
        app_xensiv_sensor_shield_bgt60ltr11_client_char_config[0] = 0;
                                               ...
                                               uint16_t len)
{
    ...
    switch (attr_handle)
    {
        case HDLD_XENSIV_SENSOR_SHIELD_BGT60LTR11_CLIENT_CHAR_CONFIG:
            /* Verify that size constraints have been met */
            if (app_xensiv_sensor_shield_bgt60ltr11_client_char_config_len >= len)
            {
                /* Value fits within the supplied buffer; copy over the value */
                memcpy(app_xensiv_sensor_shield_bgt60ltr11_client_char_config,
                     p_val,
                     len);
